...
| 宣言（施策a 段1） | 選択肢と設問文は `src/lib/declaration.ts` の1か所だけに置く。DBにはコードで入れる。**申込フォームで取った宣言を `responses` へ書き戻さない**（書き戻すと申込者が全員「宣言あり」になり、宣言の有無で申込率を比べられなくなる） | 集客戦略マップ.md §3.8 |
