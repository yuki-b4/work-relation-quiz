...
| 宣言（施策a 段1） | 選択肢・設問文・約束の文は `src/lib/declaration.ts` の1か所だけに置く。DBにはコードで入れる。**宣言を取るのは結果画面直後のフォークだけ**（申込フォームでは聞かない。両方で取ると申込者が全員「宣言あり」になり、宣言の有無で申込率を比べられなくなる。取りこぼしは体験セッションの場で聞く） | 集客戦略マップ.md §3.8・§4.2 |
| フォークの出し方 | **CTAを押した人だけに出さない。** 押さないまま読み終えた／手が止まった人にもモーダルで出す（自動は1回だけ・閉じたら二度と出さない）。**画面は差し替えない**（読んでいる最中に結果を取り上げない）。出したことは `declare_prompted_at` / `declare_prompt_via` に残す | 集客戦略マップ.md §3.6・§8.3 |
| ブリッジ（宣言の直後） | 分岐するのは「悩みの理由」の1行（場面×相手）だけ。証拠のトリセツとタイプ名は**結果カードのDOMから複製**して、2つ目の文面ソースを作らない。**原因を内面に置かず、強みと落とし穴を必ず並べる**。ガイドの中身を約束しすぎない（「あなたとあの人の力学」は段3の見立て） | 集客戦略マップ.md §3.8 |
