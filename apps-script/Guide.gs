 ...
 *
 * 【2026-07-29・配信停止中】読み解きガイドはアプリ内で完結する形に移行し、メール登録を
 * 廃止した。あわせて「指標v2」から**「メールアドレス」列を削除**したため、送信系
 * （enrollGuideLead／sendGuideBatch／backfillGuideEnroll）はこのままでは動かない。
 * 配信を再開するときは、先に Code.gs の HEADERS へ「メールアドレス」を戻し、
 * シート側にも同じ位置で列を復活させること。
 */
...
var GUIDE_CONFIG_KEYS = [
  ['sender_name',      'Mikata事務局 齋藤 祐希', '送信者名。Day1冒頭の名乗り・メールの差出人名になる'],
