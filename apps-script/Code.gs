...
// ▼ 並びはスプレッドシート「指標v2」の1行目と完全に一致させること。
//   ensureHeader が1行目をこの配列で上書きし、handleUpdate は indexOf で列位置を決めるため、
//   ズレるとデータが別の列に入る。シートの列を足す・消す・並べ替えたら、必ずここも直して再デプロイする。
//   2026-07-29：メールアドレス／相手の想定タイプ／個別読み解き希望／立場 を削除し、並びを現状に合わせた。
//   （メール登録の廃止・第三章の廃止で書き込み元が無くなったため。Guide.gs の配信を復活させる場合は
//     「メールアドレス」列の復活が要る）
var HEADERS = [
  'タイムスタンプ', 'モード', '紹介元',
  'Q1_本音', 'Q2_衝突', 'Q3_重心',
  ...
  '判定_本音', '判定_衝突', '判定_重心',
  'タイプコード', 'タイプ名',
  // 5軸スコア（0〜1。0=左の極／1=右の極／0.5=中央）。フロントの radar から記録。
  '5軸_本音', '5軸_任せ方', '5軸_境界', '5軸_摩擦', '5軸_間合い',
  // 検証アンケート（?mode=feedback のときだけ書き込まれる）
  'これは私だ', '他者当てはめ', '他者当てはめ_誰', '見せたい・話したい', '見せたい_誰', '深掘りしたい', '滑った部分',
  '回答ID',
  // 「紹介者名」「備考」はスプレッドシート側で手入力する運用列。位置を保持して壊さないため明示する。
  // 「自由記述」「理想_解消後の毎日」は②紹介の商談前ヒアリング（すべて任意）の書き込み先。
  '紹介者名', '自由記述', '理想_解消後の毎日', '備考',
  // 受け手の立場を表すセグメント（general／referral／leader-lp／member）。?ref= から判定してフロントが送る。
  'セグメント',
  // 3日間読み解きガイドの配信記録（Guide.gs が使用。配信は停止中。guide-3day-spec.md §9）
...
function handleCreate(sheet, data) {
  ...
  var values = {
    ...
    '自由記述': data.relationship || '',
    '回答ID': data.id || '',
    // 紹介者名・備考は手入力列のため新規行では空のまま。
    'セグメント': data.seg || ''
  };
