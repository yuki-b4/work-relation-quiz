...
- `prototype.html` — 検証用のプロトタイプ（スタンドアロンのHTML/CSS/JS）。`?ref=` から4セグメントを判定して出し分ける（`general`＝refなし／`referral`＝紹介者コード／`leader-lp`＝`corp-lp`・`wedding-lp`・`coffee-lp`／`member`＝`team_` 始まり。`?seg=` で上書き可）。検証アンケート5問は `?mode=feedback` を明示したときだけ表示。②紹介では結果画面から**アプリ内の読み解きガイド（全5章・本の体裁）**へ進み、終章で商談前ヒアリング3項目（すべて任意）と体験セッション申込（外部フォーム・`SESSION_FORM_URL`）に着地する。メールアドレスの登録は取らない（クリックだけで読み進められる）。①一般・③④にはガイドCTAを出さない
...
- `guide-3day-spec.md` — 「3日間の読み解きガイド」の仕様（v2：自然体・関係非依存の個人文脈）。共通ガワ（§8）とタイプ別固定文24本（`ガイド文面24本.md`）は**現在アプリ内ガイド（`prototype.html` の全5章）の文面ソース**。**メール配信（§9・`apps-script/Guide.gs`）は2026-07-27に停止**し、稼働セグメントはない（①一般はライブ経由、②紹介はアプリ内完結）。仕様と実装はそのまま温存する。着地は体験セッション → 個別コーチングセッション。
- `ガイド文面24本.md` — タイプ別本文24本（8タイプ×3日＝アプリ内ガイドの第一〜第三章）の正。文言を変えるときはここを直し、`prototype.html` の `GUIDE_BODY` と `apps-script/GuideContent.gs` に反映する。
