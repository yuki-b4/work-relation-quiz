...
- `prototype.html` — 検証用のプロトタイプ（スタンドアロンのHTML/CSS/JS）。`?ref=`/`?mode=lead` で紹介者向け（深掘り診断の申込）に切替。結果画面に「チーム力学レポート」申込フックを常設（`action:corpLead`・`mode:team-report` で法人リードシートに記録。立場＝経営者/管理職でリード文を切替、`?ref=team-` 経由の受診では非表示。詳細は `チームレポート仕様.md` §1.1）
